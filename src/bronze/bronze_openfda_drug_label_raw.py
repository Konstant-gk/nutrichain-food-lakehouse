...
def _resolve_api_key(config: OpenFdaIngestionConfig) -> Optional[str]:
    ...
    if config.secret_scope and config.secret_key:
        try:
            return dbutils.secrets.get(config.secret_scope, config.secret_key)  # type: ignore[name-defined]
