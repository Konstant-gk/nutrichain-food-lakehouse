...
**Consumption:** Power BI via Databricks SQL on Gold (`fact_product_nutrition`, `pipeline_audit`)  
**Orchestration:** Airflow DAG `nutrichain_openfood_daily` (every 3 hours UTC) → Databricks Jobs → dbt in Airflow container

**Timeline:** Portfolio build (iterative)  
...
This project builds an end-to-end **batch lakehouse**: paginated API ingest, raw landing on a Unity Catalog Volume, Bronze/Silver Delta tables, a **Gold star schema** built with dbt, automated tests at ingest and model layers, and a Power BI dashboard for pipeline health and category nutrition reporting.
...
**Our role as data engineers:** Own the pipeline from API → lakehouse → star schema → BI, with documented quality rules at every layer and no manual copy-paste in the critical path.
...
5. Deploys Databricks job code via **Git-backed Repos sync** on merge to `main` — no manual workspace uploads  

**Definition of done:** A full DAG run succeeds end-to-end; Gold tests pass; `pagination_state.json` advances; analysts can query `fact_product_nutrition` and `pipeline_audit` without touching Bronze JSON.
...
**Repo layout (high level):** `src/openfood/` (ingest), `airflow/` (DAG + Compose), `databricks/bronze|silver/`, `dbt/`, `tests/`, `powerbi/`, `.github/workflows/ci_cd.yml`.
...
**`pipeline_audit`:** Aggregates Silver by `ingest_run_id` — product counts, mismatch %, missing nutrition %, invalid EAN %, NOVA and Nutri-Score distributions — for `powerbi/pipeline-health-dashboard.pbix`.
...
| **Ingest cadence** | Every 3 hours UTC (`0 */3 * * *`) |
...
| **BI** | Power BI on Gold + audit mart |
...
- **Operations visibility** — `pipeline_audit` answers “what did last night’s run ingest and how healthy was it?” in Power BI.  
- **Any SQL-capable BI tool** can connect to Gold Delta tables; no asterisk footnotes about “clean this column first.”  
...
| Airflow DAG | `airflow/dags/openfood_bronze_dag.py` |
| Bronze job | `databricks/bronze/bronze_ingestion.py` |
| Silver job | `databricks/silver/silver_transform.py` |
